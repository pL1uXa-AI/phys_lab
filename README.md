...
| Статья | Время |
|---|---|
| Пять шагов физики | 16.0 мс |
| Отрисовка кадра | 5.2 мс |
| — из них слой связей | 2.6 мс |
| — частицы и графики | 2.6 мс |

Отсюда видно, почему вынос физики в поток дал бы больше, чем оптимизация
рендера: **физика дороже отрисовки втрое**, а слой связей — самая дешёвая
часть картинки из возможных улучшений. Замер сделан скриптом
`scripts/dev-profile.mjs`, а не на глаз; в headless-режиме абсолютные
значения завышены софтверным растеризатором, но пропорция показательна.
