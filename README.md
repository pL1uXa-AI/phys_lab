...
![Phys Lab: кристалл плавится, связи ближних соседей, графики T, E, g(r), S(k), MSD](docs/images/interface.png)

> **Что на кадре.** То же плавление, но с включённым слоем **связей ближних
> соседей** — и именно он превращает «россыпь кружков» в структуру. Видно, что
> ближний порядок ещё держится (атомы связаны, у каждого ~12 соседей), но
> решётка уже поехала. Цвет частицы — её скорость: синие медленные, красные
> быстрые.
>
> Пять графиков внизу считают то, что видно глазами:
> **T*** — выход на заданную температуру; **энергии** K, U, E;
> **g(r)** — радиальная функция, три размытых пика вместо резких;
> **S(k)** — структурный фактор с пиком ≈ 28 (у идеального кристалла он ≈ 90,
> у жидкости ≈ 2); **MSD** — среднеквадратичное смещение, по наклону которого
> считается коэффициент диффузии D.
>
> **Кадр воспроизводится командой** `npm.cmd run showcase` — она собирает все
> витринные кадры заново и падает с ненулевым кодом, если сцена оказалась
> пустой. Поэтому картинки в README не нужно обновлять вручную.

### Как читается структура

| Кристалл (T* = 0.15): правильная решётка | Плавление: решётка «поплыла» |
|---|---|
| ![Кристалл крупным планом](docs/images/lattice.png) | ![Плавление](docs/images/melting.png) |

> **Связи — это не украшение, а измерение.** Каждая линия соединяет пару
> атомов ближе 1.45σ. У ГЦК-кристалла таких пар ровно 12 на атом — это
> координационное число, определённая величина, а не «примерно». У жидкости
> связи постоянно рвутся (в среднем 9.8), у газа линий почти нет. Разброс
> длин связей (0.05 у кристалла против 0.15 у жидкости) показывает, насколько
> решётка искажена.
...
| `npm.cmd test` | Юнит-тесты ядра (235 тестов) |
| `npm.cmd run bench` | Замеры производительности ядра |
| `npm.cmd run smoke` | Сквозная проверка в настоящем Chrome (73 проверки) |
...
* **Связи ближних соседей** — слой, без которого кристалл и жидкость на
  экране неотличимы. У ГЦК-кристалла выходит ровно 12 связей на атом
  (координационное число), у жидкости — рвущаяся сетка, у газа линий нет.
  Радиус связи регулируется ползунком.
* **Шлейфы траекторий** у нескольких десятков меченых частиц: у кристалла —
  клякса вокруг узла, у жидкости — длинная извилистая линия. Это самый
  быстрый способ УВИДЕТЬ, есть ли диффузия.
* **Векторы скоростей** — короткие штрихи по направлению движения.
* **Коэффициент самодиффузии D** по наклону MSD, с оценкой по множеству начал
  отсчёта. Кристалл: D ≈ 2·10⁻⁶, жидкость при T* = 1.1: D ≈ 0.116.
* **Структурный фактор S(k)** — отличает фазы резче, чем g(r): у кристалла
  узкие высокие пики (S ≈ 90 на векторе обратной решётки), у жидкости один
  широкий горб (S ≈ 3).
* **Сохранение и загрузка состояния** в JSON. Восстановленный мир продолжает
  ТУ ЖЕ траекторию: сохраняется и состояние генератора случайных чисел,
  без которого термостат Ланжевена пошёл бы другим путём.
* **Экспорт данных**: история измерений, g(r) и S(k) в CSV (открывается в
  Excel), графики в PNG.
* **Физика в отдельном потоке (Web Worker).** Шаги интегрирования считаются
  вне главного потока, поэтому интерфейс не подвисает при перетаскивании на
  больших системах. Состояние передаётся буферами **по владению**, без
  `SharedArrayBuffer`: это сохраняет работу собранной версии с `file://`.
  Если воркер недоступен, приложение автоматически считает локально.
* **Интерактив**: слайдеры T*, ρ*, числа частиц; кнопки «нагреть», «сжать»,
  «заморозить»; протяжка мышью создаёт локальное возмущение.
* **Эксперимент: свип по температуре.** Система проходит набор T* при
  постоянной плотности, и строится кривая E(T) вместе с теплоёмкостью C_v(T).
  Пик C_v и расхождение ветвей нагрева и охлаждения отмечают фазовый переход.
* **Графики**: температура, энергии (K, U, E), g(r), S(k), MSD.
...
src/
  core/        ЯДРО. Ничего не знает про DOM, Pixi и localStorage.
    ...
    bonds.ts       сеть связей ближних соседей, координационное число
    structure.ts   структурный фактор S(k) — «рентгенограмма» системы
    diffusion.ts   среднеквадратичное смещение и коэффициент D
    snapshot.ts    сохранение и загрузка состояния (JSON, бит-в-бит)
  ...
  render/      PixiJS: сцена, камера, палитра, графики.
    lines.ts       слой отрезков (связи, шлейфы, векторы) — один батч
    trails.ts      кольцевой буфер траекторий меченых частиц
    export.ts      CSV и PNG для внешних программ
...
* Рост стоимости **линейный**: на 2 048 частиц шаг 1.1 мс, на 20 000 — 12.8 мс,
  то есть ×11 при ×9.6 частиц. Квадратичный рост дал бы ×100.
* Предел одного потока — примерно 50 000 частиц при dt = 0.004. Физика уже
  вынесена в Web Worker (см. ниже), поэтому на 20 000 частиц главный поток
  свободен для отрисовки и ввода; дальше упирается в пропускную способность
  самого воркера.

**Что из чего состоит.** Замерено в браузере (N = 2 048, ρ* = 0.7):

| Статья | Время |
|---|---|
| Пять шагов физики (локально) | 16.0 мс |
| Отрисовка кадра | 5.2 мс |
| — из них слой связей | 2.6 мс |
| — частицы и графики | 2.6 мс |

Отсюда и был сделан вынос физики в отдельный поток: **физика дороже отрисовки
втрое**, поэтому оптимизация рендера дала бы меньше. Замер сделан скриптом
`scripts/dev-profile.mjs`, а не на глаз; в headless-режиме абсолютные значения
завышены софтверным растеризатором, но пропорция показательна.

**Что это дало.** Физика вынесена в Web Worker, и приложение работает через
него. Пропускная способность воркера в зависимости от размера порции шагов
(тот же скрипт, N = 2 048):

| Порция шагов | 1 | 2 | 4 | 8 | **16** | 32 | 48 |
|---|---|---|---|---|---|---|---|
| шагов в секунду | 18 | 33 | 67 | 142 | **190** | 84 | 31 |

Максимум — **190 шагов в секунду при порции 16**. После неё пропускная
способность падает втрое: очередь команд перестаёт разгружаться, и воркер
тратит время на отставание, а не на физику. Именно поэтому автоподстройка
ограничивает порцию сверху числом 16 — это измерение, а не догадка.

Важно понимать, что порция больше НЕ выбирается по «сколько шагов влезает в
кадр»: в режиме воркера физика кадр не блокирует, и такой критерий разгонял
порцию до 21+, загоняя очередь. Критерий — «сколько шагов укладывается в
интервал между кадрами по измеренной воркером стоимости шага».

Обратное давление обязательно: без него главный поток заказывает шаги
быстрее, чем воркер считает, очередь растёт, и «Пауза» перестаёт
останавливать движение (в очереди ждут сотни уже заказанных шагов). В полёте
держится не больше двух порций, а «Шаг» игрока выполняется всегда.
...
| S(k) | `S(k) = (1/N)·|Σ exp(i k·r)|²` | «Рентгенограмма»: пики на векторах обратной решётки |
| MSD | `⟨|r(t) − r(0)|²⟩` | Копится по множеству начал отсчёта |
| D | `D = наклон MSD / 6` | Соотношение Эйнштейна в трёх измерениях |
| Связи | пары ближе 1.45σ | Координационное число: у ГЦК ровно 12 |

### Три величины, которые различают фазы

Одного взгляда на сцену мало, чтобы понять, что происходит с системой,
поэтому каждая фаза описывается числом:

| | Кристалл (T* = 0.15) | Жидкость (T* = 1.1) | Газ (T* = 1.6) |
|---|---|---|---|
| **Связи на атом** | 12.00 | 9.8 | < 1 |
| **Пик g(r)** | 7.1 | 2.5 | 1.8 |
| **Пик S(k)** | ≈ 90 | ≈ 2 | ≈ 1 |
| **D (диффузия)** | ≈ 2·10⁻⁶ | ≈ 0.12 | ≈ 1 |

Кристалл и жидкость различаются по D **в десятки тысяч раз** — это и есть
количественный ответ на вопрос «атомы стоят или двигаются», который по
картинке даёт только качественное впечатление.

> **Окно подгонки MSD выбирается по данным.** Раньше оно было жёстким
> (20…80 % максимального лага) — эвристика, которая молча врёт, если
> баллистический участок длиннее 20 %. Замерено на газе при ρ* = 0.06:
> локальный наклон `d ln MSD / d ln t` равен 1.5–1.55 на первых лагах вместо
> 1, и жёсткое окно захватывало эту часть. Сдвиг начала окна с 1.75τ на 3.25τ
> изменил D с 1.49 на 1.29 — разница между «эффективным» и истинным
> коэффициентом. Теперь `diffusion()` ищет самый длинный отрезок, где
> логарифмический наклон близок к 1.

> **Важно про D = 0.** Ноль у настоящего кристалла и ноль до набора
> статистики — разные вещи. Окно наблюдения MSD заполняется за тысячи шагов,
> и до этого подгонка идёт по пустым бинам. Поэтому интерфейс показывает не
> «D = 0», а прогресс набора («набор 54 %»), пока данных мало.

> **Где стоит пик S(k).** На векторе **обратной** решётки, а не на расстоянии
> 2π/r_сосед. Для ГЦК при ρ* = 0.95 низший разрешённый вектор — (111):
> `|G₁₁₁| = 2π√3/a = 6.74`. Величина `2π/r_nn = 5.50` — это положение пика
> g(r); S(k) есть фурье-образ g(r), и её максимум сдвинут. Путать эти два
> числа легко, поэтому тест проверяет их по отдельности.

### Эксперимент: где именно плавится

Свип по температуре при постоянной плотности даёт кривую E(T) и
теплоёмкость C_v(T); пик C_v и расхождение ветвей нагрева и охлаждения
отмечают переход.

Здесь есть ловушка, о которой стоит знать читателю: **однородный кристалл
перегревается**. Равновесная температура плавления объёмного
Леннард-Джонса при ρ* = 0.95 равна T* ≈ 0.7, но плавление начинается с
зародыша — свободной поверхности или дефекта. Внутри идеальной решётки с
периодическими границами зародыша нет, и она остаётся метастабильной.
Измерено: скачок энергии приходится на T* ≈ 1.25, то есть решётка «терпит»
перегрев почти вдвое.

Это не ошибка расчёта, а измеримое свойство: гистерезис перехода первого
рода. Чтобы увидеть равновесную температуру, ведите **охлаждение** от
жидкости — переохлаждение выражено слабее перегрева.
...
Уровень 1 доведён до состояния, когда физика верна (силы, энергия, ансамбли,
устойчивость дискретизации), производительность линейна и измерена, структура
видна на экране, а результаты можно вынести в файл. В `ROADMAP.md` расписаны
уровни 2 и 3: молекулы с химическими реакциями (пружинные связи, правила
реакций, автокатализ) и макрофизика (твёрдые тела, SPH-жидкости, сыпучие
среды). Они строятся на тех же численных методах.

Ближайшие осмысленные шаги, по убыванию пользы:

1. **Web Worker для физики.** Ядро не знает про DOM — его можно вынести почти
   как есть. Уберёт подтормаживание интерфейса на больших системах. Критерий
   приёмки уже готов: тест на бит-в-бит воспроизводимость траектории должен
   остаться зелёным.
2. **Уровень 2: молекулы.** Пружинные связи и правила реакций. Ключи `type` и
   `charge` у частиц уже заложены в `core/types.ts`.
3. **Частные g(r) и S(k) по типам пар** (A–A, A–B, B–B) — понадобится для
   ионных систем.
