 ...
 *
 * Экспортируется наружу намеренно: этим же квантилем пользуется клиент
 * воркера, когда считает границы цветовой шкалы из полученного кадра. Две
 * отдельные реализации неизбежно разошлись бы — и заметить это можно было
 * бы только глазами, по «поехавшим» цветам.
 */
...
export class World {
  ...
  /**
   * Сколько шагов выполнено за ВСЁ время жизни мира — монотонно.
   *
   * `steps` обнуляется при пересборке системы (это часть описания текущей
   * траектории), а этот счётчик — нет. Нужен интерфейсу, чтобы честно
   * показывать фактическую скорость расчёта: при смене числа частиц и
   * плотности мгновенная скорость не должна «прыгать» в ноль вместе со
   * сбросом `steps`.
   */
  ...
  /**
   * Разброс длин связей относительно равновесной 2^(1/6)σ.
   *
   * Ноль — идеальная решётка, где связи одной длины. Рост означает искажение:
   * тепловое расширение, плавление, дефекты. Это количественная замена фразе
   * «на картинке видно, что решётка поехала».
   */
  get bondSpread(): number {
    return this.bondNetwork().lengthSpread();
  }
  ...
  /**
   * Границы цветовой шкалы для ПРОИЗВОЛЬНОГО массива величин.
   *
   * Нужно клиенту воркера: он получает готовые массивы (скорость, смещение,
   * плотность) в кадре и должен посчитать по ним ту же шкалу, что посчитал
   * бы мир. Логика вынесена в отдельный метод, чтобы формулы не разошлись:
   * дублировать квантиль в клиенте — значит однажды получить разные цвета
   * при одних данных.
   *
   * @param values значения по частицам
   * @param alive  маска живых частиц
   */
  static colorRange(
    values: Float64Array,
    ...
    const n = Math.min(values.length, alive.length);
    let min = Number.POSITIVE_INFINITY;
    let max = Number.NEGATIVE_INFINITY;
    for (let i = 0; i < n; i++) {
      if (alive[i] === 0) continue;
    ...
    if (max > min) max = quantileOf(values, n, alive, 0.995, min, max);
    if (max - min < 1e-9) max = min + 1e-9;
    return { min, max };
  }
