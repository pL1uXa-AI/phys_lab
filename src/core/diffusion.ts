...
export class MeanSquareDisplacement {
   ...
   * значит D = наклон / 6.
   *
   * ─── Почему окно подгонки выбирается ПО ДАННЫМ, а не долями ─────────────
   *
   * Раньше окно было жёстким: от 20 % до 80 % максимального лага. Это
   * эвристика, и она молча врёт в двух случаях, которые встречаются на
   * практике:
   *
   *   1. **Баллистический участок длиннее 20 %.** Частица летит прямолинейно
   *      примерно время между столкновениями. Если окно начинается внутри
   *      этого участка, наклон занижен, а r² падает. Так выглядит, например,
   *      газ при низкой плотности: на замеренной кривой наклон в лог-лог
   *      координатах равен 1.5–1.55 на первых лагах вместо 1.
   *   2. **Разные режимы на разных лагах.** В пористой или кластерной
   *      структуре MSD сначала растёт как √t (субдиффузия, наклон 0.5), и
   *      только потом выходит на линейный режим. Жёсткое окно захватывает
   *      оба режима сразу, и подгонка даёт «эффективный» D вместо истинного
   *      — r² при этом падает до 0.6…0.8.
   *
   * Поэтому окно ищется так: считается локальный логарифмический наклон
   * `d ln MSD / d ln t`, и берётся САМЫЙ ДЛИННЫЙ отрезок, где он близок к 1
   * и меняется слабо. Это и есть определение диффузионного режима
   * (MSD ∝ t), а не догадка о том, где он должен начинаться.
   *
   * ─── Гарантия разумности ────────────────────────────────────────────────
   *
   * Если линейного участка нет вовсе (кристалл: MSD выходит на плато,
   * наклон → 0), честнее вернуть ноль и НИЗКИЙ r², чем подогнать прямую под
   * изогнутую кривую и выдать маленькое, но «уверенное» D. Кристалл — это
   * отсутствие диффузии, и результат обязан это показывать.
   *
  ...
  diffusion(): DiffusionResult {
    ...
    // Бины с данными. Пустые — это нули, они «притянули» бы наклон к нулю.
    const usable: number[] = [];
    for (let k = 0; k <= last; k++) if (counts[k] > 0) usable.push(k);
    if (usable.length < 4) return { D: 0, r2: 0, lagRange: [0, 0] };

    const window = this.linearWindow(lag, msd, usable);
    if (!window) {
      return { D: 0, r2: 0, lagRange: [lag[usable[0]], lag[usable[usable.length - 1]]] };
    }

    const { slope, r2 } = linearFit(lag, msd, window.index);
    const D = slope > 0 ? slope / 6 : 0;
    return {
      D,
      r2,
      lagRange: [lag[window.index[0]], lag[window.index[window.index.length - 1]]],
    };
  }

  /**
   * Поиск диффузионного участка по локальному наклону в лог-лог координатах.
   *
   * Возвращает подмножество бинов для линейной подгонки или null, если
   * участка с наклоном, близким к 1, не нашлось.
   */
  private linearWindow(
    lag: Float64Array,
    msd: Float64Array,
    usable: number[],
  ): { index: Int32Array } | null {
    /*
     * Локальный наклон между соседними ИСПОЛЬЗУЕМЫМИ бинами.
     *
     * Соседними по списку, а не по индексу: пропущенные бины дали бы
     * деление на ноль в логарифме шага.
     */
    ...
    for (let k = 1; k < n; k++) {
      ...
      const cur = usable[k];
      const dt = Math.log(lag[cur] / lag[prev]);
      const dy = Math.log(Math.max(1e-12, msd[cur]) / Math.max(1e-12, msd[prev]));
      slope[k] = dt > 1e-9 ? dy / dt : 0;
    }

    /*
     * Насколько наклон может отличаться от 1.
     *
     * 0.35 — компромисс по замерам: у чистой жидкости локальный наклон
     * колеблется в пределах ±0.15 вокруг единицы (шум конечной статистики),
     * а субдиффузия в кластерной структуре даёт 0.5–0.7, то есть уверенно
     * отсекается. Слишком узкий коридор отбросил бы и хорошие данные.
     */
    ...
    let from = -1;
    for (let k = 1; k <= n; k++) {
      const inBand = k < n && Math.abs(slope[k] - 1) <= TOLERANCE;
      if (inBand && from < 0) from = k;
      if (!inBand && from >= 0) {
        // Участок [from, k-1] закончился.
        if (bestFrom < 0 || k - 1 - from > bestTo - bestFrom) {
          bestFrom = from;
          bestTo = k - 1;
        }
        from = -1;
    ...
    // Нужно хотя бы 4 точки, иначе наклон определяется шумом.
    if (bestFrom < 0 || bestTo - bestFrom < 3) {
      /*
       * Линейного участка нет. Это ожидаемый случай для кристалла: MSD
       * выходит на плато. Но именно там важно показать почти нулевое D,
       * поэтому для КРИСТАЛЛА (малый абсолютный рост MSD) берём хвост кривой
       * — он даёт маленький наклон и низкий r², что честно отражает
       * отсутствие диффузии. Для жидкости такой запасной путь не нужен:
       * у неё линейный участок есть всегда.
       */
      const tail = n > 6 ? Math.floor(n * 0.5) : 0;
      const index = Int32Array.from(usable.slice(tail));
      if (index.length < 4) return null;
      return { index };
    }

    // В подгонку включаем и границы найденного плато.
    const selected = usable.slice(bestFrom, bestTo + 1);
    if (selected.length < 4) return null;
    return { index: Int32Array.from(selected) };
  }
