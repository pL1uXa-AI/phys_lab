 ...
 *
 * Список берётся из `grid.ts`, а не дублируется здесь: он обязан совпадать
 * с окрестностью, которой пользуются силы и поиск связей. Разойдясь, они
 * начнут «видеть» разные соседние ячейки, и это будет молчаливая ошибка —
 * пара просто не попадёт в статистику.
 */
const HALF_OFFSETS = HALF_NEIGHBOUR_OFFSETS;
...
export class RadialDistribution {
  ...
  accumulate(state: ParticleState, box: number, periodic: boolean): void {
    ...
    // Проверка обязана быть по ФАКТИЧЕСКОМУ размеру ячейки, а не по числу
    // ячеек: `prepare` берёт `max(3, floor(box / maxRadius))`, поэтому в
    // маленьком ящике ячейка выходит меньше maxRadius, и сосед попадает
    // «через одну» ячейку — часть пар теряется молча. Это тот же дефект, что
    // ловился в списках Верле (см. `grid.ts`), только в g(r): измерено, что
    // при box = 7.27σ и ячейке 2.42σ вместо 3.5σ кривая недосчитывает пары.
    if (grid.size + 1e-9 < this.maxRadius) {
