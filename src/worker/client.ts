...
export class PhysicsWorkerClient {
   ...
   *
   * Вызывается раз в кадр отрисовки. Буферы предыдущего кадра возвращаются
   * воркеру: после передачи по владению они здесь бесполезны, а память
   * жалко — на 20 000 частиц это около мегабайта.
   */
