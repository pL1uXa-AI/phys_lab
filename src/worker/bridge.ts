...
export class PhysicsBridge {
   ...
   *
   * Ось передаётся командой — так же, как у кисти, и по той же причине: её
   * берут из камеры главного потока, а в воркере камеры нет.
   *
   * @returns число замороженных частиц в локальном режиме; в режиме воркера
   *   `-1`, потому что команда выполняется асинхронно и число заранее
   *   неизвестно. Раньше здесь возвращался 0, и это означало «не сработало»
   *   буквально: команда вообще не отправлялась, и кнопка была мертва.
   *   Признак «отправлено» отличается от «заморожено 0 частиц» намеренно:
   *   путать эти случаи — тот же дефект, что и с нулями в CSV.
   */
