...
export type WorkerCommand =
  ...
  | { type: 'unfreezeAll' }
  /**
   * Заморозить ОБЛАСТЬ: цилиндр вдоль оси взгляда.
   *
   * Ось передаётся так же, как у кисти, и по той же причине: цилиндр задаётся
   * осью взгляда из главного потока, а её в воркере взять неоткуда. Раньше
   * команда просто не отправлялась, и `freezeRegion` возвращал 0 — то есть
   * кнопка «Заморозить» в режиме воркера не работала вовсе.
   */
  | {
      type: 'freezeRegion';
...
export interface FrameSummary {
  count: number;
  /**
   * Число ЖИВЫХ частиц (внутри ящика).
   *
   * Отдельно от `count`, потому что это разные величины при открытых
   * границах: улетевшие частицы `removeEscaped` помечает мёртвыми, но из
   * массивов они не исчезают, поэтому `count` остаётся прежним.
   *
   * Без этого поля зеркало подставляло в `measurement.count` общее число, и
   * проверки уровней «испарилось не меньше 40 частиц» и «осталось не меньше
   * N» в режиме воркера ВСЕГДА получали ноль: убыль считалась как
   * `startCount − count`, то есть как `N − N`. Измерено на уровне 3: воркер
   * не проходил уровень за 90 с, тогда как локально он проходился за 21 с.
   */
  aliveCount: number;
  box: number;
  time: number;
  ...
  diffusionR2: number;
  /**
   * Границы участка, по которому подогнана прямая MSD.
   *
   * Нужны графику: по ним подсвечивается отрезок фита, и связь «кривая → D»
   * становится видимой. Зеркало раньше отдавало вместо них весь диапазон
   * лагов — тот же класс дефекта, что и потерянные столбцы истории: величина
   * не терялась совсем, но показывала неправду.
   */
  diffusionLagStart: number;
  diffusionLagEnd: number;
  /** Готово ли окно MSD. */
  msdReady: boolean;
  msdProgress: number;
  /**
  ...
  params: WorldParams;
  /**
   * Сколько шагов воркер выполнил за всё время — МОНОТОННО.
   *
   * Не путать с `steps`: тот сбрасывается при пересборке системы, потому что
   * описывает текущую траекторию. Здесь же нужен счётчик, по которому
   * главный поток может понять, сколько ЗАКАЗАННЫХ шагов уже сделано: на нём
   * держится ограничение очереди (см. `PhysicsBridge.backlog`).
   */
  stepsExecuted: number;
  /**
   * Измеренная воркером стоимость одного шага, мс.
   *
   * Автоподстройка числа шагов на кадр в главном потоке не может измерить
   * эту величину сама: шаги идут в другом потоке. Без неё она измеряла
   * стоимость `postMessage` (доли миллисекунды) и разгоняла число шагов
   * вверх — вместе с очередью, из-за которой пауза «не работала».
   */
  stepCostMs: number;
}
